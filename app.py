import re, asyncio, time, uuid, io
...
def parse_board_dates(html: str) -> List[datetime]:
    ...
    for m in DATE_RE.finditer(html):
        ...
        dt = datetime(y, mo, d, hh, mm, tzinfo=KST)  # interpret as KST
        out.append(dt)
    return out
...
async def count_board(client: httpx.AsyncClient, code: str, pages: int, delay_ms: int) -> Dict[str, int]:
    ...
    today_s = ymd(now_kst())
    yday_s = ymd(now_kst() - timedelta(days=1))
...
async def extract_codes(client: httpx.AsyncClient, market_pages: int, topn: int, delay_ms: int) -> List[str]:
    ...
    for sosok in (0, 1):
        for p in range(1, market_pages + 1):
            ...
            if not ok:
                continue
            d = max(delay_ms, 80)  # minimum 80ms
...
async def fetch_name(client: httpx.AsyncClient, code: str) -> str:
    try:
        ...
        if isinstance(d, dict) and d.get("stockName"):
            return str(d["stockName"])
    except Exception:
        pass
    try:
...
async def run_job(job_id: str, params: Dict[str, Any]):
    try:
        ...
        async with httpx.AsyncClient(headers=HEADERS, timeout=30, limits=limits) as client:
            ...
            set_progress(job_id, f"todayreturn 수집 중… (TOPK {len(rankTop)}개)")
            async def _ret(r):
                async with sem:
                    v = await fetch_today_return(client, r["code"])
                    if v is not None:
                        r["todayreturn"] = float(v)
                    return r
            rankTop = await asyncio.gather(*[_ret(dict(r)) for r in rankTop])

            set_progress(job_id, f"todayreturn 수집 중… (전체 {len(board_res)}개)")
            board_res = await asyncio.gather(*[_ret(r) for r in board_res])

        ts = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")
        dur_s = time.perf_counter() - t0
        if dur_s >= 3600:
            dur = f"{int(dur_s//3600)}시간 {int(dur_s%3600//60)}분 {int(dur_s%60)}초"
        elif dur_s >= 60:
            dur = f"{int(dur_s//60)}분 {int(dur_s%60)}초"
        else:
            dur = f"{int(dur_s)}초"
...
async def api_run(payload: Dict[str, Any]):
    job_id = new_job()
    asyncio.create_task(run_job(job_id, payload or {}))
